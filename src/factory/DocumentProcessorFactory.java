...
public abstract class DocumentProcessorFactory {
    ...
    protected abstract DocumentProcessor createProcessor(DocumentType documentType, FileFormat fileFormat);

    public DocumentProcessor getProcessor(DocumentType documentType, FileFormat fileFormat, String documentName) {
    ...
    protected void validateFileFormat(DocumentType documentType, FileFormat fileFormat, String documentName) {
        List<FileFormat> allowed = getAllowedFormats(documentType);
        if (!allowed.contains(fileFormat)) {
            throw new UnsupportedFileFormatException(getCountry(), documentType, fileFormat, allowed, documentName);
        }
    }

    @Override
    public String toString() {
        return String.format("DocumentProcessorFactory[%s — %s]",
                getCountry().name(), getCountry().getFullName());
    }
}
